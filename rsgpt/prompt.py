...
Your answer should always be about querying the RediSearch index as described in the CONTEXT below.
...
REPLY_PROMPT = {
  PromptFormat.ARGS: '''Your answer must be encoded as a single unformatted line.
...
The third string must always be the query.
Any additional strings must be the query's remaining arguments, in the order these are expected.''',
  PromptFormat.CLI: '''Your answer must be a RediSearch query encoded a as single unformatted line of text.
This format is the same as that of the input expected by the redis-cli tool.''',
  PromptFormat.PYTHON: '''Your answer must be a snippet of Python code that constructs the operation using redis-py.''',
  PromptFormat.JSON: '''Your answer must be a JSON list of strings.
The first string must always be the command's name.
The second string must always be the index's name.
The third string must always be the query.
Any additional strings must be the query's remaining arguments, in the order these are expected.''',
...
def get_chat_prompt(format: PromptFormat,) -> ChatPromptTemplate:
...
def get_system_prompt(format: PromptFormat,) -> ChatPromptTemplate:
  return ChatPromptTemplate.from_template(
    SYSTEM_PROMPT +
    '\n\nFORMAT:\n' + REPLY_PROMPT[format] +
    '\n\nCONTEXT:\n{context}'
