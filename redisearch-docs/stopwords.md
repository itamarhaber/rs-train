...
github_path: docs/docs/advanced-concepts/stopwords.md
...
Stop words for an index can be defined (or disabled completely) on index creation using the `STOPWORDS` argument with the [[`FT.CREATE`](/commands/ft.create) command.
...
Disabling stop words completely can be done by passing `STOPWORDS 0` to [`FT.CREATE`](/commands/ft.create).
